...
class OneChannelTwoMarkerGenerator(SequenceGenerator):
    # TODO: make these generic, preferably extractable from broadbean sequence object or smth
    number_of_markers = 2
    number_of_channels = 1
...
class RepeatingStaircaseRampGeneratorTwoTriggers(OneChannelTwoMarkerGenerator):
    """
    Generates broadbean sequence for a staircase with two marker signals for
    triggering.
    """
    ...
    def __init__(self,
        ...
        # TODO: somewhere MAX VALUE has to meet the AWG marker channel number limit
        self.add_parameter(name='marker_falling_channel_number',
                           label='Marker falling channel number',
                           unit='',
                           get_cmd=None,
                           set_cmd=None,
                           get_parser=int,
                           ...
                           vals=Numbers(min_value=1),
                           docstring="Number of the channel for the marker "
                                     "signal for the falling edge within the analog channel"
                           )
        self.add_parameter(name='marker_rising_channel_number',
                           label='Marker rising channel number',
                           unit='',
                           get_cmd=None,
                           set_cmd=None,
                           get_parser=int,
                           ...
                           vals=Numbers(min_value=1),
                           docstring="Number of the channel for the marker "
                                     "signal for the rising edge within the analog channel"
                           )
    ...
    def make_broadbean_sequence(self) -> broadbean.Sequence:
        ...
        for step_number, step_voltage in enumerate(
            ...
            # the digital trigger signal with a falling edge
            staircase_blueprint.setSegmentMarker(
                step_segment_name,
                (0, self.settlement_time()),
                self.marker_falling_channel_number()
            )
            
            # the digital trigger signal with a rising edge
            staircase_blueprint.setSegmentMarker(
                step_segment_name,
                (self.settlement_time(), self.lockin_integration_time()),
                self.marker_rising_channel_number()
            )
